...
namespace MotionMatching.Gameplay
{
	...
	public class LogicContactState : LogicState
	{
		...
		private void HandleTimeScaling()
		{
			...
			float distanceToSetedContact = 0;
			float distanceToAnimationContact = 0;
			switch (Features.TimeScalingPositionMask)
			{
				case ContactStateTimeScalingPositionMask.X:
					{
						distanceToSetedContact = Mathf.Abs(transform.position.x - m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.x);
						distanceToAnimationContact = Mathf.Abs(transform.position.x - globalSpaceContact.position.x);
					}
					break;
				case ContactStateTimeScalingPositionMask.Y:
					{
						distanceToSetedContact = Mathf.Abs(transform.position.y - m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.y);
						distanceToAnimationContact = Mathf.Abs(transform.position.y - globalSpaceContact.position.y);
					}
					break;
				case ContactStateTimeScalingPositionMask.Z:
					{
						distanceToSetedContact = Mathf.Abs(transform.position.z - m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.z);
						distanceToAnimationContact = Mathf.Abs(transform.position.z - globalSpaceContact.position.z);
					}
					break;
				case ContactStateTimeScalingPositionMask.X | ContactStateTimeScalingPositionMask.Y:
					{
						Vector2 transformPos = new Vector2(transform.position.x, transform.position.y);
						distanceToSetedContact = Vector2.Distance(
							transformPos,
							new Vector2(
								m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.x,
								m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.y
								)
							);
						distanceToAnimationContact = Vector2.Distance(
							transformPos,
							new Vector2(
								globalSpaceContact.position.x,
								globalSpaceContact.position.y
								)
							);
					}
					break;
				case ContactStateTimeScalingPositionMask.X | ContactStateTimeScalingPositionMask.Z:
					{
						...
						distanceToSetedContact = Vector2.Distance(
							...
							new Vector2(
								m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.x,
								m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.z
								)
							);
						distanceToAnimationContact = Vector2.Distance(
							transformPos,
							new Vector2(
								globalSpaceContact.position.x,
								globalSpaceContact.position.z
								)
							);
					}
					break;
				case ContactStateTimeScalingPositionMask.Y | ContactStateTimeScalingPositionMask.Z:
					{
						Vector2 transformPos = new Vector2(transform.position.y, transform.position.z);
						distanceToSetedContact = Vector2.Distance(
							transformPos,
							new Vector2(
								m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position.y,
					...
					break;
				case ContactStateTimeScalingPositionMask.X | ContactStateTimeScalingPositionMask.Y | ContactStateTimeScalingPositionMask.Z:
					{
						distanceToSetedContact = Vector3.Distance(
							transform.position,
							m_LogicLayer.globalSpaceContacts[destinationContactIndex].frameContact.position
							);

						distanceToAnimationContact = Vector3.Distance(
							transform.position,
							globalSpaceContact.position
							);
					}
					break;
			}
